...
import org.backmeup.logic.impl.helper.EntityManagerFactoryProducer;
...
public class BusinessLogicImplTest {
	...
	public static void setUp() {
		try {
			...
			*/
			Weld weld = new Weld();
			System.out.println(EntityManagerFactoryProducer.class);
	...
	public void testCreateBackupJob() {
		/*try {
			...
			SourceSinkDescribable source = sources.get(0);

			List<SourceSinkDescribable> sinks = logic.getDatasinks();
			assert (sinks.size() > 0);
			SourceSinkDescribable sink = sinks.get(0);

			System.out.println("Register source:");
			AuthRequest ar = logic.preAuth(u.getUsername(), source.getId(),
					"A Profile", true, "hi");
			String url = ar.getRedirectURL();
			System.out.println(url);
			URLReader.launchBrowser(url);
			System.out.print("code: ");
			String code = URLReader.waitForResult();//readLine();
			...
			System.out.println("Register sink:");
			AuthRequest sinkRequest = logic.preAuth(u.getUsername(),
					sink.getId(), "A Profile", false, "hi");
			url = sinkRequest.getRedirectURL();			
			System.out.println(url);
			URLReader.launchBrowser(url);
			System.out.print("code: ");
			code = URLReader.waitForResult();
			//code = readLine();
			p = new Properties();
			entries = code.split("&");
			for (String entry : entries) {
			...
			sourcesList.add(sourceProfileId);
