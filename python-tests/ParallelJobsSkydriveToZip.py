...
def createJob(numberOfJobs, bmu, user, pwd):
  ...
  sourceId = bmu.auth_datasource(user, "org.backmeup.skydrive", name, pwd).data["profileId"]
  bmu.update_profile(sourceId, SKYDRIVE_AUTH, pwd);

  sinkId = bmu.auth_datasink(user, "org.backmeup.zip", "SinkProfile", pwd).data["profileId"]

  for i in range(0, numberOfJobs):
    res = bmu.create_backup_job(user, pwd, [sourceId], ["org.backmeup.indexer"], sinkId, "monthly", "Skydrive To Zip: " + name)
    if res.code >= 400:
      print res.data
