...
class TestMetadata(TestCase):
  ...
  def test_get_specific_metadata(self):
    ...
    result2 = update_profile(profileId, 
        {KEY_SOURCE_TOKEN : SOURCE_TOKEN, KEY_SOURCE_SECRET : SOURCE_SECRET}, "pass1")
    
    # get metadata for unknown user; should not work!
    result = get_specific_metadata("UnknownUser", profileId, "META_BACKUP_FREQUENCY", "pass1")
    self.assertEquals(result.code, httplib.BAD_REQUEST)
    
    # get metadata for unknown profile; should not work!
    result = get_specific_metadata("UnknownUser", 99999, "META_BACKUP_FREQUENCY", "pass1")
    self.assertEquals(result.code, httplib.BAD_REQUEST)

    # get BACKUP_FREQUENCY
    result = get_specific_metadata("TestUser", profileId, "META_BACKUP_FREQUENCY", "pass1")
    ...
    result = get_specific_metadata("TestUser", profileId, METADATA_TEST_SPECIFIC_META, "pass1")
  ...
  def test_get_metadata(self):
    ...
    # get metadata for unknown user; should not work!
    result = get_metadata("UnknownUser", profileId, "pass1")
    self.assertEquals(result.code, httplib.BAD_REQUEST)
    
    # get metadata for unknown profile; should not work!
    result = get_metadata("UnknownUser", 99999, "pass1")
    self.assertEquals(result.code, httplib.BAD_REQUEST)

    # get BACKUP_FREQUENCY
    result = get_metadata("TestUser", profileId, "pass1")
    self.assertIn("metadata", result.data)
    self.assertIn("META_BACKUP_FREQUENCY", result.data["metadata"])
    self.assertEquals(result.data["metadata"]["META_BACKUP_FREQUENCY"], METADATA_TEST_EXPECTED_FREQUENCY)
