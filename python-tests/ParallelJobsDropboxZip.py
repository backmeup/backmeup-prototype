...
def createJob(numberOfJobs, bmu, user, pwd):
  ...
  sourceId = bmu.auth_datasource(user, "org.backmeup.dropbox", name, pwd).data["profileId"]
  bmu.update_profile(sourceId, DROPBOX_DATA, pwd);

  sinkId = bmu.auth_datasink(user, "org.backmeup.zip", name, pwd).data["profileId"]
  for i in range(0, numberOfJobs):
    res = bmu.create_backup_job(user, pwd, [sourceId], ["org.backmeup.indexer"], sinkId, "monthly", "Dropbox to Zip: " + name)
    if res.code >= 400:
      print res.data
